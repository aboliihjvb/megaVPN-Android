...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/configs" && (request.method === "POST" || request.method === "PUT")) {
      ...
      const now = Date.now();
      await env.DB.prepare(`INSERT INTO configs(id,name,type,content,enabled,created_at,updated_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,type=excluded.type,content=excluded.content,enabled=excluded.enabled,updated_at=excluded.updated_at`)
        .bind(id, String(body.name), String(body.type || "auto"), String(body.content), body.enabled === false ? 0 : 1, now, now).run();
